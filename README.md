...
Hazenix Blog 是一个现代化的个人博客系统，采用前后端分离架构，支持文章发布、评论互动、用户管理等功能。项目使用 Spring Boot + Vue 3 技术栈，提供流畅的阅读体验和完善的内容管理功能。

### 核心功能

- 📝 **文章管理**：支持 Markdown 编辑、文章分类、标签管理、置顶功能
- 💬 **评论系统**：多级评论、点赞、表情支持
- 👤 **用户系统**：多种登录方式（手机号、邮箱、第三方登录）、JWT 认证
- 🔍 **搜索功能**：文章全文搜索、标签筛选
- 🎨 **主题切换**：支持深色/浅色主题
- 🤖 **AI 功能**：文章 AI 摘要生成
- 🔗 **友链管理**：友情链接申请与管理
- 📊 **数据统计**：文章浏览量、点赞数、收藏数统计
...
本项目采用 **Spring Boot + Vue 3 高度定制的博客系统**
...
| 技术                | 说明                  | 版本    | 备注                                       |
| ------------------- | --------------------- | ------- | ------------------------------------------ |
| `Spring Boot`       | Spring 快速集成脚手架 | 2.6.13  | https://spring.io/projects/spring-boot     |
| `MyBatis Plus`      | MyBatis 增强工具      | 3.5.3.1 | https://baomidou.com/                      |
| `Lombok`            | 实体类增强工具        | 1.18.20 | https://github.com/rzwitserloot/lombok     |
| `Knife4j`           | 接口文档工具          | 3.0.2   | https://gitee.com/xiaoym/knife4j           |
| `JJWT`              | JWT 处理库            | 0.9.1   | https://github.com/jwtk/jjwt               |
| `MySQL Driver`      | MySQL 数据库驱动      | 8.0+    | https://dev.mysql.com/downloads/connector/ |
| `Redis`             | 缓存中间件            | 6.2.7   | https://redis.io/                          |
...
| 技术         | 说明         | 版本    | 备注                          |
| ------------ | ------------ | ------- | ----------------------------- |
| `Vue`        | 前端框架     | 3.5+    | https://v3.vuejs.org/         |
| `Vue-Router` | 路由框架     | 4.6+    | https://router.vuejs.org/     |
| `Vite`       | 前端构建工具 | 7.2+    | https://vitejs.dev/           |
| `Axios`      | HTTP 客户端  | latest  | https://axios-http.com/       |
| `Markdown`   | Markdown 编辑器 | latest | 支持文章编辑                  |
...
| `Redis Desktop Manager` | Redis 可视化管理工具 | latest | https://github.com/qishibo/AnotherRedisDesktopManager |
...
| password        | varchar(255)  | 密码               | 存储哈希值，不存明文                    |
| phone           | varchar(11)   | 手机号             | 唯一索引 uk_phone                       |
| email           | varchar(100)  | 邮箱               | 唯一索引 uk_email，可用于订阅文章       |
| avatar          | varchar(1000) | 头像图片路径       | 存储 OSS 路径                           |
| gender          | tinyint       | 性别               | 0保密/未知 1男 2女                      |
| status          | tinyint       | 账号状态           | 0正常 1锁定                             |
| last_login_time | datetime      | 最后登录时间       | 用于安全审计                            |
| role            | tinyint       | 角色               | 0管理员 1作者 2用户（默认2）            |
| create_time     | datetime      | 创建时间           |                                         |
| update_time     | datetime      | 最后修改时间       |                                         |

**索引设计：**
- `uk_phone`：手机号唯一索引，加快登录查询
- `uk_email`：邮箱唯一索引，用于邮箱登录和找回密码
...
| platform    | varchar(10)  | 平台类型                 | WECHAT, QQ, GITHUB, WEIBO |
| content     | varchar(200) | 联系方式内容             | 50字以内 |
| create_time | datetime     | 创建时间                 |      |
| update_time | datetime     | 最后修改时间             |      |

#### user_article（用户文章关联表）
...
| is_liked    | tinyint(1) | 是否点赞   | 0未点赞 1点赞    |
| is_favorite | tinyint(1) | 是否收藏   | 0未收藏 1收藏    |
...
| -------------- | ------------- | ------------ | ------------------------------------- |
| id             | bigint        | 主键         | 自增                                  |
| user_id        | bigint        | 作者 ID      | 外键                                  |
| title          | varchar(255)  | 标题         | 必填                                  |
| content        | text          | 文章内容     | Markdown 格式                         |
| cover_image    | varchar(600)  | 封面图片路径 | OSS 路径                              |
| tags_id        | int           | 文章标签 ID  |                                       |
| tags_name      | varchar(100)  | 文章标签名   | 冗余字段，提升查询性能                |
| category_id    | int           | 文章分类 ID  |                                       |
| like_count     | int           | 点赞数       | 默认0                                 |
| favorite_count | int           | 收藏数       | 默认0                                 |
| view_count     | int           | 浏览数       | 默认0                                 |
| slug           | varchar(255)  | URL 标识符   | 用于 SEO 友好的 URL                   |
| is_top         | tinyint       | 是否置顶     | 0不置顶 1置顶                         |
| status         | tinyint       | 状态         | 0正常 1待审核 2草稿                   |
| create_time    | datetime      | 创建时间     |                                       |
| update_time    | datetime      | 最后修改时间 |                                       |

**设计说明：**
- 图片存储在 OSS，数据库只存路径
- slug 字段用于生成 SEO 友好的 URL，如：`/article/spring-boot-tutorial`

#### article_tags（文章标签关联表）

| 字段名     | 数据类型     | 说明       | 备注                     |
| ---------- | ------------ | ---------- | ------------------------ |
| tags_id    | int          | 标签 ID    | 不设为主键，允许重复     |
| tags_name  | varchar(100) | 标签名     | 冗余字段                 |
| article_id | bigint       | 文章 ID    |                          |

**注意：** tags_id 不设为主键且不自动递增，以支持同一标签下的多篇文章。
...
| id          | int          | 主键          | 自增                             |
| name        | varchar(32)  | 分类名        |                                  |
| sort        | tinyint(1)   | 排序          | 用于排序                         |
| slug        | varchar(255) | URL 标识符    |                                  |
| status      | tinyint(1)   | 状态          | 0正常 1未启用                    |
| create_time | datetime     | 创建时间      |                                  |
| update_time | datetime     | 最后修改时间  |                                  |
...
| user_id        | bigint      | 用户 ID            | 外键                         |
| username       | varchar(32) | 用户名             | 冗余字段                     |
| reply_id       | bigint      | 回复的评论 ID      | 如果是第一级评论则为 null    |
| reply_username | varchar(32) | 回复的用户名       | 冗余字段                     |
| content        | text        | 评论内容           |                              |
| like_count     | int         | 点赞数             | 默认0                        |
...
#### 配置建议
- **CPU/内存**：2C2G 以上（动态网站推荐配置）
- **带宽**：5M 以上（保证前端资源流畅加载）
- **地域**：
  - 国内服务器：需要 ICP 备案
  - 香港服务器：无需备案，推荐个人开发者使用

#### 安全防护
- **CDN 加速**：使用 CDN 隐藏真实服务器 IP，防止 DDoS 攻击
- **流量控制**：后端使用 Redis 实现 IP 访问频率限制
- **防火墙**：配置服务器防火墙规则
